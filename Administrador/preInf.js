...
$(document).ready(function () {
    ...
    coleccionProductos.on("child_added", datos => {
        dataSet = [datos.key, datos.child("nombre").val(), datos.child("apellido").val(), datos.child("edad").val(),datos.child("direccion").val(),datos.child("telefono").val(),datos.child("curp").val(),datos.child("fecha").val(), datos.child("tutor").val(), datos.child("estatus").val()];
    ...
    $('form').submit(function (e) {
        ...
        data = { clave: productos.getKey(), nombre: nombre, apellido: apellido, edad: edad, direccion: direccion,telefono: telefono, curp: curp, fecha:fecha, tutor: tutor, estatus: estatus };
        actualizacionData = {};
    ...
    $("#tablaPreinscripcion").on("click", ".btnEditar", function () {
        ...
        let curp = ($(this).closest('tr').find('td:eq(6)').text());
        let fecha = ($(this).closest('tr').find('td:eq(7)').text());
        let tutor = ($(this).closest('tr').find('td:eq(8)').text());
        let estatus = ($(this).closest('tr').find('td:eq(9)').text())
        $('#id').val(id);
        $('#nombre').val(nombre);
        $('#apellido').val(apellido);
        $('#edad').val(edad);
