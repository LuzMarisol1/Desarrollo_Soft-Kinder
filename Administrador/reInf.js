$(document).ready(function () {
    ...
    var coleccionProductos = db.ref().child("reinscripcion");

    var dataSet = [];//array para guardar los valores de los campos inputs del form
    var table = $('#tablaReinscripcion').DataTable({
        language: {
            "decimal": "",
        ...
        columnDefs: [
            {
                targets: [0],
                visible: true, //ocultamos la columna de ID que es la [0]                        
            },
            {
                targets: -1,
    ...
    coleccionProductos.on("child_added", datos => {
        dataSet = [datos.key, datos.child("nombre").val(), datos.child("apellido").val(), datos.child("edad").val(), datos.child("direccion").val(), datos.child("grado").val(),datos.child("estatus").val()];
    ...
    $('form').submit(function (e) {
        ...
        data = { clave: productos.getKey(), nombre: nombre, apellido: apellido, edad: edad,direccion: direccion, grado: grado, estatus: estatus };
        actualizacionData = {};
    ...
    $("#tablaReinscripcion").on("click", ".btnEditar", function () {
        filaEditada = table.row($(this).parents('tr'));
        let fila = $('#tablaReinscripcion').dataTable().fnGetData($(this).closest('tr'));
        let id = fila[0];
        let nombre = $(this).closest('tr').find('td:eq(1)').text();
        let apellido = $(this).closest('tr').find('td:eq(2)').text();
        let edad = parseInt($(this).closest('tr').find('td:eq(3)').text());
        let direccion = ($(this).closest('tr').find('td:eq(4)').text());
        let grado = ($(this).closest('tr').find('td:eq(5)').text());
    ...
    //para eliminar
    $("#tablaReinscripcion").on("click", ".btnBorrar", function () {
        filaEliminada = $(this);
        Swal.fire({
            ...
            if (result.value) {
                let fila = $('#tablaReinscripcion').dataTable().fnGetData($(this).closest('tr'));
                let id = fila[0];
                db.ref(`reinscripcion/${id}`).remove()
                Swal.fire('¡Eliminado!', 'El registro del alumno ha sido eliminado.', 'success')
            }
        })
    });
